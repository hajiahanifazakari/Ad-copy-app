...
// REPLACE your current generateAdCopyVariants function with this:
async function generateAdCopyVariants() {
    // 1. Safely grab DOM elements
    const prodNameInput = document.getElementById("input-prod-name");
    const prodDescInput = document.getElementById("input-prod-desc");
    const toneSelect = document.getElementById("input-prod-tone");
    const apiKeyInput = document.getElementById("api-key-input");

    // Defensive check: Make sure elements actually exist on the page
    if (!prodNameInput || !prodDescInput) {
        console.error("Critical HTML binding missing. Check your element IDs.");
        return;
    }

    const prodName = prodNameInput.value.trim();
    const prodDesc = prodDescInput.value.trim();
    const toneChoice = toneSelect ? toneSelect.value : "conversion";
    const apiKey = apiKeyInput ? apiKeyInput.value.trim() : "";

    // 2. Validate user inputs
    if (!prodName || !prodDesc) {
        alert("Action Required: Please provide both a Brand Name and a Product Description before generating.");
        return;
    }

    // 3. UI Loading Feedback Subsystem
    const triggerButton = document.querySelector("button[onclick='generateAdCopyVariants()']");
    let originalBtnHTML = "<span>Generate Split Variants</span>";
    
    if (triggerButton) {
        originalBtnHTML = triggerButton.innerHTML;
        triggerButton.disabled = true;
        triggerButton.innerHTML = `<span class="flex items-center justify-center gap-2"><i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Processing Simulation Matrix...</span>`;
        if (window.lucide) lucide.createIcons();
    }

    // 4. Fallback Router Engine
    try {
        if (apiKey && apiKey !== "") {
            // Attempting actual API calls
            await executeActualAnthropicTransaction(apiKey, prodName, prodDesc, toneChoice, state.activePlatformChannel);
        } else {
            // The Key is blank -> Bypassing network requests seamlessly
            console.log("Anthropic key field is blank. Activating High-Fidelity Simulator...");
            
            // Artificial delay to make the UI transition feel realistic (1.2 seconds)
            await new Promise(resolve => setTimeout(resolve, 1200)); 
            
            injectHighFidelityMockPayload(prodName, prodDesc, toneChoice, state.activePlatformChannel);
        }
    } catch (apiError) {
        console.error("API Router Anomaly: falling back to simulator", apiError);
        injectHighFidelityMockPayload(prodName, prodDesc, toneChoice, state.activePlatformChannel);
    } finally {
        // 5. Restore UI Trigger Controls
        if (triggerButton) {
            triggerButton.disabled = false;
            triggerButton.innerHTML = originalBtnHTML;
            if (window.lucide) lucide.createIcons();
